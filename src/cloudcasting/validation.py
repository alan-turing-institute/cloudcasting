...
def get_validation_params():
    ...
    val_config_dir = (
        Path(os.path.dirname(cloudcasting.__file__)).parent.parent / "configs/validation"
    )
...
def _calc_ssim_sample(input, target, win_size: int | None = None):
...
def calc_ssim(input: np.ndarray, target: np.ndarray, win_size: int | None = None):
...
def validate_model(
    ...
    for t0 in pd.to_datetime(val_config.sample_video_t0_times):
        ...
        for channel_name, channel_index in zip(
            val_config.sample_video_channels, channel_indexes, strict=False
        ):
